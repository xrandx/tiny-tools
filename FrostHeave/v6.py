...
#   最小二乘法
def least_square(former_days_list, InputDataClass):
...
def get_k0(InputDataClass, flag_day_num, last_data, src_data):
    ...
    #   d > 16
    if recorded_day_count > flag_day_num + 1:
        return fmt(last_data.k0), fmt(last_data.k0_packet_num)
    else:
        #   16 >= d >= 3
        if recorded_day_count >= 3:
            if last_data.date == src_data.date:
                return fmt(last_data.k0), int(last_data.k0_packet_num)
            else:
                query_data = session.query(InputDataClass.date) \
                    .order_by(InputDataClass.id.desc()) \
                    .distinct(InputDataClass.date) \
                    .limit(recorded_day_count)
                ...
                recent_data_list = session.query(InputDataClass) \
                    ...
                    .all()
                k0 = least_square(recent_data_list, InputDataClass)
                return k0, len(recent_data_list)
        else:
            # d <= 2
            return None
...
def get_param(src_data, InputDataClass):
    ...
    if last_data is None:
        return None
    tmp = get_k0(InputDataClass, flag_day_num, last_data, src_data)
    if tmp is None:
        return None
    else:
        k0, packet_num = tmp
        mutation = (src_data.strain - fmt(last_data.mutation_accumulate) - fmt(last_data.strain)) - (fmt(k0) * (src_data.temperature - fmt(last_data.temperature)))
        mutation = fmt(mutation)
        delta_t = abs(src_data.temperature - fmt(last_data.temperature))
        if delta_t < 1e-6:
            deviation = True
        else:
            deviation = abs(mutation / delta_t) - 180 > 1e-6

        former_days_list = session.query(InputDataClass) \
            .order_by(InputDataClass.id.desc()) \
            .limit(packet_num)
        k = least_square(former_days_list, InputDataClass)

        truth = is_time_less3h(src_data.date, src_data.time, last_data.date, last_data.time) \
                and (abs(mutation) - 400 > 1e-6)  \
                and deviation

        if truth:
            return k0, packet_num, k, fmt(last_data.mutation_accumulate) + mutation
        else:
            return k0, packet_num, k, fmt(last_data.mutation_accumulate)
...
def compute_save(data, InputDataClass):
    ...
    # None : < 3 days
    if tmp is not None:
        k0, packet_num, k, mutation_accumulate = tmp
        data.height = (-k + k0) * 0.5 + mutation_accumulate * 0.0189
        data.tsf = (-k + k0) * 0.6 + 10 + mutation_accumulate * 0.08475
        data.strain -= mutation_accumulate

        data.stress = 0.21 * 11.8 * (data.temperature - data.tsf)

        data.k0 = k0
        data.k0_packet_num = packet_num
        data.k = k
        data.mutation_accumulate = mutation_accumulate

    fmt = "%s|%s|%s|%s|%s|%s" % (InputDataClass.__tablename__, data.date, data.time,
